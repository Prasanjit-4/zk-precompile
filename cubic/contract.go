...
var (
	...
	// CubicRawABI contains the raw ABI of Cubic contract.
	//go:embed contract.abi
	CubicRawABI string

	CubicABI = contract.ParseABI(CubicRawABI)

	CubicPrecompile = createCubicPrecompile()
)
...
func UnpackVerifyInput(input []byte) (VerifyInput, error) {
	...
	err := CubicABI.UnpackInputIntoInterface(&inputStruct, "verify", input)
...
func UnpackVerifyOutput(output []byte) (bool, error) {
	res, err := CubicABI.Unpack("verify", output)
	if err != nil {
		return false, err
	}
...
// createCubicPrecompile returns a StatefulPrecompiledContract with getters and setters for the precompile.

func createCubicPrecompile() contract.StatefulPrecompiledContract {
	...
	for name, function := range abiFunctionMap {
		method, ok := CubicABI.Methods[name]
		if !ok {
